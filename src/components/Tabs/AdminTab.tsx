...
function ConnectionsSection() {
    ...
    useEffect(() => {
      ...
      const handleMessage = (event: MessageEvent) => {
        if (event.data?.type === 'CJ_CONNECTED') {
          ...
          setStatus({ type: 'success', message: 'Bridge successfully synchronised.' });
          quickLog('✅ Synchronisation received from Mesh');
        }
      };
    ...
    const handleConnectCJ = async () => {
      const k = cjKey.trim();
      if (!k) {
        setStatus({ type: 'error', message: 'Please provide your API Authorisation Key.' });
        return;
      }

      setStatus({ type: 'loading', message: 'Establishing Secure Bridge...' });
      quickLog('⚡ Handshaking via Aura Gateway...');
      
      try {
        // Use the dedicated server-side connect route for the handshake
        ...
        if (result.success) {
          ...
          setStatus({ type: 'success', message: 'Aura Secure Bridge established! System is online.' });
          quickLog('✅ SECURE LINK ACTIVE');
          quickLog(`📡 Protocol: ${result.message}`);
        } else {
          setStatus({ type: 'error', message: result.message || 'Authorisation failed.' });
          quickLog(`❌ Access Denied: ${result.message}`);
          updateSettings({ cjConnected: false });
        }
      } catch (err: any) {
        setStatus({ type: 'error', message: 'Network timeout or aura gateway unreachable.' });
    ...
    const handleDisconnectCJ = () => {
      if (confirm('Deactivate all supplier automation protocols?')) {
    ...
    const checkConnection = async () => {
      setStatus({ type: 'loading', message: 'Performing health check...' });
      quickLog('🧪 Running system check...');
      try {
        ...
        if (health.status === 'healthy') {
          ...
          setStatus({ type: 'error', message: `Health Check Failed: ${health.message}` });
          quickLog(`⚠️ Health Check: FAILED (${health.message})`);
          updateSettings({ cjConnected: false });
        }
      } catch(err: any) {
        setStatus({ type: 'error', message: 'Health check failed due to network error.' });
      ...
      <div className="space-y-8 max-w-4xl">
        <div className="flex justify-between items-end">
          <div>
            ...
            <p className="text-gray-400">Manage secure bridges to global supply chains from our AI gateway.</p>
          </div>
          <AnimatePresence>
            {status.type && (
              <motion.div 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 bg-[#141414] border border-white/5 rounded-2xl overflow-hidden shadow-2xl flex flex-col">
            <div className="p-8 border-b border-white/5">
               <div className="flex justify-between items-center mb-8">
                  ...
                  <div className="flex items-center gap-3 bg-black/40 px-4 py-2 rounded-full border border-white/5">
                    ...
                    <span className={`text-[11px] font-black uppercase tracking-[2px] ${settings.cjConnected ? 'text-[#50C878]' : 'text-gray-500'}`}>
                      {settings.cjConnected ? 'Online' : 'Offline'}
                    </span>
                  </div>
               </div>
               ...
               <div className="space-y-6">
                  <div>
                    <div className="flex justify-between items-center mb-2">
                      <label className="text-[10px] text-gray-500 uppercase tracking-[3px] font-black">API Authorisation Key</label>
                    ...
                    <div className="relative">
                      <input 
                        ...
                        disabled={settings.cjConnected || status.type === 'loading'}
                        placeholder="••••••••••••••••••••••••••••"
                  ...
                  <div className="pt-4 flex items-center gap-4">
                    {!settings.cjConnected ? (
                      <button 
                        ...
                        className="flex-1 py-4 gold-gradient text-black font-black text-xs uppercase tracking-[3px] rounded-xl hover:brightness-110 active:scale-[0.98] transition-all disabled:opacity-50 flex items-center justify-center gap-2"
                      >
                        {status.type === 'loading' && <Loader2 className="w-4 h-4 animate-spin" />}
            ...
            <div className="flex-1 bg-black/40 min-h-[160px] p-6 font-mono text-[10px] text-gray-500 overflow-y-auto border-t border-white/5">
              ...
              <div className="space-y-1.5">
                {log.map((msg, i) => (
                  <div key={i} className={`flex gap-3 ${msg.includes('❌') || msg.includes('FAILED') || msg.includes('Denied') ? 'text-red-400' : msg.includes('✅') || msg.includes('SUCCESS') ? 'text-green-400' : 'text-gray-500'}`}>
                     <span className="shrink-0 opacity-30 tracking-tighter">[{i.toString().padStart(2, '0')}]</span>
                     <span>{msg}</span>
                  </div>
                ))}
                {log.length === 0 && <div className="italic opacity-30 uppercase tracking-[2px]">Awaiting initial connection attempt...</div>}
              </div>
            </div>
          </div>
          ...
          <div className="space-y-6">
            <div className="bg-[#141414] border border-[#D4AF37]/20 p-6 rounded-2xl shadow-xl shadow-gold/5">
               <h4 className="text-[#D4AF37] font-bold uppercase tracking-[2px] text-xs mb-4">Initialisation Guide</h4>
               <ul className="space-y-4">
                  {[
                    { step: "01", text: "Log in to your master CJ account dashboard." },
                    { step: "02", text: "Navigate to API Settings and copy your Authorisation Key." },
                    { step: "03", text: "Paste the key into the Aura Bridge above." },
                    { step: "04", text: "System will run a validation handshake to link access." }
                  ].map((item, i) => (
            ...
            <div className="bg-[#141414] border border-white/5 p-6 rounded-2xl opacity-60 grayscale hover:grayscale-0 hover:opacity-100 transition-all cursor-not-allowed">
               ...
               <p className="text-[10px] text-gray-500 uppercase font-black tracking-widest leading-loose">Automated payouts coming soon to British merchants.</p>
            </div>
          </div>
        </div>
        ...
        <div className="mt-12 bg-[#1A1A1A] border border-[#D4AF37]/30 rounded-3xl overflow-hidden shadow-2xl relative group">
          ...
          <div className="p-8 border-b border-white/5 flex justify-between items-center bg-[#1A1A1A]">
            <div>
              ...
              <p className="text-xs text-gray-500 uppercase tracking-[2px] mt-1 font-bold italic">Alternate Connection Mesh Protocol</p>
            </div>
          </div>
          <div className="p-0">
